...
const main=async(privateKeys)=>{
    ...
    await Promise.all(tasks_qiandao);
    ...
    const tasks=privateKeys.map(privateKey=>
        limit(async ()=>{
            ...
            if(privateKeys.length<20){
                ...
                const LP_goonUSD_amount=ethers.parseEther((Math.floor(goonUSD_Stable_LP_amount)-30).toString());
...
main(NewPrivatKeys(PrivateKeys$136Wallets))