...
const {NewPrivatKeys,sleep,provider,getRandomUniqueIndices,convertNumToHexa,walletSendtxData}=require('../../util/common.cjs')
...
const main=async(privateKeys)=>{
    ...
    const Plume_provider=await provider(RPC.plumerpc);
    for (let index = 0; index < privateKeys.length; index++) {//PrivateKeys$18Wallets.length
        let Plume_wallet=new ethers.Wallet(privateKeys[index],Plume_provider);
